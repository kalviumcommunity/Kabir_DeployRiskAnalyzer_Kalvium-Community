...
def ingest_csv(filepath, delimiter=',', encoding='utf-8', dtype_dict=None):
    ...
    try:
        ...
        print(f"[SUCCESS] CSV loaded: {filepath}")
...
def ingest_json(filepath, is_nested=False):
    ...
    try:
        if is_nested:
            ...
            print("[SUCCESS] Nested JSON flattened to tabular format")
        else:
            df = pd.read_json(filepath)
        
        print(f"[SUCCESS] JSON loaded: {filepath}")
        print(f"  Shape: {df.shape[0]} rows x {df.shape[1]} columns")
        return df
    except FileNotFoundError:
        print(f"Error: File not found - {filepath}")
    ...
    for delimiter in delimiters:
        for encoding in fallback_encodings:
            try:
                ...
                print(f"[SUCCESS] Successfully loaded with delimiter='{delimiter}', encoding='{encoding}'")
                return df
...
if __name__ == "__main__":
    ...
    print("\n[SUCCESS] All data ingested and saved to processed/")
